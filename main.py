...
def generate_dataset(url):
    ...
    title = url[
        len("https://opendata.socrata.com/" + category + "/") : url.rindex("/")
    ].lower()
    directory = "datasets/" + category + "/" + title
    if not os.path.exists(directory + "/data/"):
        os.makedirs(directory + "/data/")
    with open(
        directory + "/data/" + title + ".csv", "w", encoding="utf-8"
    ) as output_file:
        number_of_values = 0
        for i, line in enumerate(urlopen(csv_url)):
            ...
            if all(value == "" for value in value_list):
                continue
    ...
    number_of_rows = 0
    with open(
        directory + "/data/" + title + ".csv", "r", encoding="utf-8"
    ) as output_file:
    ...
    data_valid = datapackage_creator(
        location="datasets/" + category + "/" + title,
    ...
    for meta in soup.find_all("meta"):
        ...
        if meta_name and "description" in meta_name:
            with open(directory + "/README.md", "w", encoding="utf-8") as output_file:
                ...
                decoded_line = (
                    content.strip().replace('"', "") if isinstance(content, str) else ""
                )
                output_file.write(decoded_line + "\n")

                output_file.write(
                    "\nThis dataset was scraped from [Socrata - "
                    + title
                    + "]("
                    + url
                    + ")"
...
def datapackage_creator(location, title, name, source_title, source_path):
    ...
    package.descriptor["licences"][0]["title"] = (
        "Open Data Commons Public Domain Dedication and Licence (PDDL)"
    )
    package.descriptor["licences"][0]["path"] = (
